...
async def analyze_dental_image(
    ...
    try:
        ...
        filename = f"{name.replace(' ', '')}{timestamp}_{dentalImage.filename}"
        ...
        if client is not None and dental_scans_collection is not None:
            ...
            logger.warning("⚠ No se pudo conectar a MongoDB - los datos no se guardaron")
...
async def registro(
    ...
    try:
        ...
        if client is not None and dental_scans_collection is not None:
            ...
            logger.warning("⚠ No se pudo conectar a MongoDB - el registro no se guardó")
